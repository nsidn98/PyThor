"""
    Twin Delayed Deep Deterministic (TD3) Policy Gradients for vector-based states
...
class DDPG(LightningModule):
    ...
    def __init__(self, hparams: argparse.Namespace) -> None:
        ...
        self.value_net1        = ValueNetwork(obs_shape, n_actions,[256], hparams.activation, hparams.init_w)
        self.value_net2        = ValueNetwork(obs_shape, n_actions,[256], hparams.activation, hparams.init_w)
        self.target_value_net1 = ValueNetwork(obs_shape, n_actions,[256], hparams.activation, hparams.init_w)
        self.target_value_net2 = ValueNetwork(obs_shape, n_actions,[256], hparams.activation, hparams.init_w)
        ...
        self.noise = GaussianExploration(env.action_space)
        
        self.buffer = ReplayBuffer(self.hparams.replay_size)

        self.agent = DDPGAgent(self.env, self.buffer, self.hparams)

        self.agent.soft_update_networks(self.value_net1,self.target_value_net1, soft_tau=1) # sync the networks
        self.agent.soft_update_networks(self.value_net2,self.target_value_net2, soft_tau=1) # sync the networks
    ...
    def value_loss1(self,batch: Tuple[torch.Tensor, torch.Tensor]):
        state, action, reward, done, next_state = batch
        done = done.type(torch.FloatTensor)

    def ddpg_policy_loss(self, batch: Tuple[torch.Tensor, torch.Tensor]):
        state, action, reward, done, next_state = batch
        done = done.type(torch.FloatTensor)
    ...
    def ddpg_value_loss(self, batch: Tuple[torch.Tensor, torch.Tensor]):
        state, action, reward, done, next_state = batch
        done = done.type(torch.FloatTensor)
        ...
        self.value_loss = nn.MSELoss()(value, expected_value.detach())
    ...
    def configure_optimizers(self):
        ...
        value_optimizer1 = optimizers[self.hparams.opt](self.value_net1.parameters(), lr=self.value_lr, weight_decay=self.hparams.weight_decay)
        value_optimizer2 = optimizers[self.hparams.opt](self.value_net2.parameters(), lr=self.value_lr, weight_decay=self.hparams.weight_decay)
