...
RLBot and TelegramBotCallback classes for the monitoring and control
...
class RLBot(object):
    ...
    Supports the following commands:
     ...
     /plot: get a reply with the rewards plot image
    ...
    def plot_diagrams(self, bot, update):
        ...
        if not self.hist_logs:
            ...
            return
        legend_keys = ['rewards']
        rewards = np.array(self.hist_logs['rewards'])
        fig = plt.figure()
        ax = plt.axes()
        ax.plot(rewards)
